/*Agregar a la clase Persona un método EsMenorQue(Persona p) que devuelva verdadero si la persona que
recibe el mensaje tiene menos edad que la persona enviada como parámetro. Utilizarlo para realizar un programa
...
public class Persona
{
    ...
    // Método público que compara la edad y el DNI de esta persona con otra
    // Devuelve true si esta persona es "menor" (más joven) que la persona 'p'
    // Reglas de comparación:
    // 1. Si la edad es menor, devuelve true
    // 2. Si las edades son iguales, devuelve true si el DNI numérico es mayor (persona más joven)
    public bool EsMenorQue(Persona p)
    {
        // Caso 1: Menor edad
        if (this._edad < p._edad) return true;

        // Caso 2: Misma edad, comparar DNI
        if (this._edad == p._edad)
        {
             // Intentamos convertir los DNI a long para comparar numéricamente
             // Esto evita errores de comparación alfanumérica (ej. "9" > "10" como texto)
            if (long.TryParse(this._dni, out long dniThis) && 
                long.TryParse(p._dni, out long dniP))
            {
                return dniThis > dniP; // Mayor DNI = "más joven"
            }
        }
...
public class Program
{
    ...
    public static void Main()
    {
        ...
        // --- Búsqueda de la persona más joven (con desempate por DNI) ---
        // Identifica la persona más joven y usa el DNI para desempatar si hay edades iguales.
        if (personas.Count > 0)
        {
            ...
            {
                // Si la persona actual es "menor" que el actual "más joven", actualizamos
                if (persona.EsMenorQue(masJoven))
                {
                    masJoven = persona;
                }
            }
            ...
            Console.WriteLine("\n--- Persona más joven ---");
            masJoven.Imprimir(1); // Usamos índice 1 para el formato de salida
        }
