...
- Eliminar los métodos: GetDiagonalPrincipal() y GetDiagonalSecundaria(),
...
public class Matriz
{
    ...
    // --- Indizador (Reemplaza a SetElemento y GetElemento) ---

    // Indizador para acceder a los elementos de la matriz usando sintaxis de corchetes: matriz[fila, columna]
    // Permite leer y escribir valores directamente.
        ...
        for (int i = 0; i < Filas; i++)
        {
            ...
            for (int j = 0; j < Columnas; j++)
            {
                ...
                if (formatString != null)
                {
                    ...
                    // Usamos el indizador para obtener el valor
                    valorTexto = this[i, j].ToString(formatString);
                }
                else
                {
                    // Convierte el número a texto usando el formato por defecto
                    // Usamos el indizador para obtener el valor
                    valorTexto = this[i, j].ToString();
                }
    ...
    public double[] GetFila(int fila)
    {
        ...
        for (int j = 0; j < Columnas; j++)
        {
            // Copia el valor de la matriz a la nueva fila usando el indizador
            filaArray[j] = this[fila, j];
        }
    ...
    public double[] GetColumna(int columna)
    {
        ...
        for (int i = 0; i < Filas; i++)
        {
            // Copia el valor de la matriz a la nueva columna usando el indizador
            columnaArray[i] = this[i, columna];
        }
        // Retorna el arreglo con los valores de la columna
        return columnaArray;
    }

    // Propiedad de solo lectura para obtener la diagonal principal (de esquina superior izquierda a inferior derecha)
    // Reemplaza al método GetDiagonalPrincipal()
    public double[] DiagonalPrincipal
    {
        get
        {
            ...
            for (int i = 0; i < Filas; i++)
            {
                // Guarda el valor en la posición (i, i) usando el indizador
                diagonal[i] = this[i, i];
            }
    ...
    // Propiedad de solo lectura para obtener la diagonal secundaria (de esquina superior derecha a inferior izquierda)
    // Reemplaza al método GetDiagonalSecundaria()
    public double[] DiagonalSecundaria
    {
        get
        {
            ...
            for (int i = 0; i < Filas; i++)
            {
                // Guarda el valor en la posición (i, Filas - 1 - i) usando el indizador
                diagonal[i] = this[i, Filas - 1 - i];
            }
    ...
    public double[][] GetArregloDeArreglo()
    {
        ...
        for (int i = 0; i < Filas; i++)
        {
            ...
            for (int j = 0; j < Columnas; j++)
            {
                // Copia el valor de la matriz a la estructura de arreglo de arreglos usando el indizador
    ...
    public void Restarle(Matriz m)
    {
        ...
        for (int i = 0; i < Filas; i++)
        {
            ...
            for (int j = 0; j < Columnas; j++)
            {
                // Resta el valor de la matriz externa al valor interno usando el campo directo de 'm'
                datos[i, j] -= m.datos[i, j];
            }
        }
    }
        ...
        for (int i = 0; i < filasResultado; i++)
        {
            ...
            for (int j = 0; j < columnasResultado; j++)
            {
                ...
                for (int k = 0; k < this.Columnas; k++)
                {
                    ...
                    // Usamos el campo directo de 'm' para acceder a sus datos
...
class Program // Clase principal, punto de entrada del programa
{
    static void Main() // Método principal, donde comienza la ejecución del programa
    {
        ...
        // Establece los valores en cada posición usando el indizador
        ...
        // Accedemos a la propiedad en lugar de llamar a un método
        double[] diagonalPrincipal = m.DiagonalPrincipal;
        Console.WriteLine("\nDiagonal Principal:");
        ...
        // Accedemos a la propiedad en lugar de llamar a un método
        double[] diagonalSecundaria = m.DiagonalSecundaria;
        Console.WriteLine("\nDiagonal Secundaria:");
        ...
        try
        {
            ...
            // Se asigna el valor a una variable local descartable para evitar el error de "statement inválido".
            // Esto fuerza la ejecución de la propiedad y la generación de la excepción.
            var _ = noCuadrada.DiagonalPrincipal; // sintaxis _ para descartar un valor que ya no se necesita
        }
...
1. Indizador (this[int fila, int columna]):
-Reemplaza a SetElemento y GetElemento.
-Ahora permite la sintaxis matriz[0, 0] = 5.5; para escribir y double valor = matriz[0, 0]; para leer.
-Ambos get y set utilizan ValidarIndices para asegurar que no haya errores de acceso.

2. Propiedades de Diagonal:
-DiagonalPrincipal y DiagonalSecundaria ahora son propiedades ({ get { ... } }) en lugar de métodos.
-Se accede a ellas sin paréntesis: var diag = matriz.DiagonalPrincipal;.
-Internamente utilizan el nuevo indizador this[i, j] para mantener consistencia.

3. Uso del Indizador en el resto de la clase:
-En los métodos que ya existían (como Imprimir, GetFila, GetColumna, GetArregloDeArreglo y las propiedades de diagonal), 
se ha actualizado el acceso a los datos para que usen el nuevo indizador this[i, j] en lugar de acceder directamente al campo privado datos[i, j].
-Esto centraliza la validación de índices. Si en el futuro se cambiára la lógica de validación, solo tiene que hacerse desde un lugar (el indizador).

Ejemplo Visual para contrastar diferencias:

- Antes (con los métodos antiguos):
Matriz m = new Matriz(2, 2);
m.SetElemento(0, 0, 10.5);
double valor = m.GetElemento(0, 0);
double[] diag = m.GetDiagonalPrincipal(); // Llamada como método

- Ahora (con el nuevo código):
Matriz m = new Matriz(2, 2);
m[0, 0] = 10.5; // Uso del indizador para escribir
double valor = m[0, 0]; // Uso del indizador para leer
double[] diag = m.DiagonalPrincipal; // Acceso como propiedad (sin paréntesis)

NOTA TÉCNICA: Uso de "var _ = noCuadrada.DiagonalPrincipal;" en la prueba de error

EL PROBLEMA:
Si intentáramos usar la sintaxis:    noCuadrada.DiagonalPrincipal;
Obtendríamos un error de compilación: 
    "Only assignment, call, increment, decrement, await, and new object expressions can be used as a statement"

¿Por qué ocurre?
En C#, acceder a una propiedad es una EXPRESIÓN que devuelve un valor. 
Una expresión por sí sola no es una SENTENCIA válida si el valor devuelto no se utiliza (ni se asigna, ni se pasa a un método, etc.).
El compilador considera que la línea "no hace nada útil" y la rechaza.

LA SOLUCIÓN:
Como el objetivo de esta línea es provocar la excepción (para probar el bloque catch) y no necesitamos el valor de retorno,
debemos forzar la ejecución de la expresión asignando su resultado al "descarte" (_).

- var _: Asigna el valor a una variable temporal anónima (con descarte).
- Esto satisface al compilador (la expresión "se usa"), pero el valor se ignora inmediatamente.
- Al evaluarse la propiedad, se ejecuta el bloque `get`, que lanza la `InvalidOperationException` esperada.
*/
